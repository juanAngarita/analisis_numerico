---
title: "Raiz cuadrada"
author: "Juan Anagrita, HectorHernandez, Aldemar"
date: "July 28,2019"
output:
  html_document:
    df_print: paged
...
raizCuadrada = function(x,n,E){
  ...
  errorRelativo <- c(errorRelativo,abs(x-y)/abs(y))
  resultadosParciales = c(resultadosParciales,y)
  lista = list("errorAbsoluto" = errorAbsoluto,"errorRelativo" = errorRelativo, "resultadosParciales" = resultadosParciales, "y" = y)
}
```
...
```{r}
plot(resultados$errorAbsoluto, type = "b", xlab="Iteraciones", ylab="Error absoluto(estimacion)", main = "Estimacion error absoluto, valor inicial = 3")
```
...
plot(resultados$errorAbsoluto, type = "b", xlab="Iteraciones", ylab="Error absoluto(estimacion)", main = "Estimacion error absoluto(valor inicial = 8)")
```
...
```{r}
plot(x = 1:10,y = iterations, type = "b", xlab="x(o)", ylab="Iteraciones necesarias", main = "iteraciones con diferentes x(0)")
```
...
**Presicion: ** El algoritmo tiene presicion hasta la 16 cifra significativa, que es el maximo de cifras significativas que trabaja r. A partir de esta cifra el valor de la raiz varia de manera erronea asi se trabaje con un error de truncamiento mayor.
